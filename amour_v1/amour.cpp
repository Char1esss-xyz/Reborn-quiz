...
int main()
{
    ...
    threshold(channels[0], binary, 245, 255, 0);                               // 二值化
    GaussianBlur(binary, Gaussian, Size(3, 3), 0);                             // 滤波
    findContours(Gaussian, contours, hierarchy, RETR_TREE, CHAIN_APPROX_NONE); // 轮廓检测
    ...
    for (int i = 0; i < contours.size(); i++)
    {
        ...
        // 除去较小轮廓
        if (area < 100 || contours[i].size() <= 1 || area > 1000)
